...
const compareCompress = (originalSize, compressedSize) => {
  ...
  return (
    <div className="App">
      <Wrapper>
        <Instruction/>
        <CardUpload upload={uploadHandler} imgFiles={files} status={status}/>
        {status.uploadImg&& <CompressButton compressing={handleCompress}/>}
        {status.clicked&& <CardDownload imgFiles={files} status={status}/>}
        {status.clicked&& <Message imgFiles={files} compareCompress={compareCompress}/>}
      </Wrapper>
    </div>
  );
}
